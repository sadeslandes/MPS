...
//    unsigned int ones, tenths = 0;
    ...
    printf("MPS Timer Interrupt Test\n\n\r");
...
void UART0_INIT(void)
{
    ...
    TMOD = 0x2D;           // Timer1, Mode 2: 8-bit counter/timer with auto-reload.
    TH1     = (unsigned char)-(SYSCLK/BAUDRATE/16); // Set Timer1 reload value for baudrate
    CKCON  |= 0x12;             // Timer1 uses SYSCLK as time base.
    TL1     = TH1;
	TH0 = 0x35;  // Reload the Timer 0 High Byte Value.
	TL0 = 0x80;  // Reload the Timer 0 Low Byte Value.
    TR1     = 1;                // Start Timer1.
	TR0		= 1;
