...
void main(void)
{
	...
	p = (__xdata unsigned char *)(0x2005);
	t = (__xdata unsigned char *)(0x2005);
	
	//z = (__xdata unsigned char *)(0x2005);
    while(1)
    {
  		*p=getchar();
		printf("*p is %c at address 0x%4x \n\r",*p,p);
		printf("*t is %c at address 0x%4x \n\n\r",*t,t);	
		
		//p+=0x100; //test if printf just doesnt properly display addresses
		// it doesn't.

		*z = 'a';
		printf("*p is now %c at address 0x%4x \n\r",*p,p);
		printf("*t is now %c at address 0x%4x \n\n\r",*t,t);	
    }
}
...
void PORT_INIT(void)
{
    ...
    P0MDOUT = 0xFF;//0x01;    // Set TX0 pin to push-pull
