...
void main(void)
{
	...
	while(1){
		printf("Enter a memory address (1FF0 - 1FFF) and a character to input\n\r0x");
		memAddr = 0;
		for(i=0;i<4;i++){
			c = getchar();
			putchar(c);
		...
		*int_ram = c;
		...
		r = *int_ram;
		for(memAddr = 0x2000; memAddr < 0x2801; memAddr++)
	    {
			...
			*ext_ram = r;
			...
			for(i=0;i<100;i++);
			printf("Value from internal xram: %c\n\rValue on external xram: %c\t0x%4x\n\n\r", c,*ext_ram,memAddr);
